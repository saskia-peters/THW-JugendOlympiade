...
function renderGroupEvaluations(evaluations) {
    ...
    let html = '<h2 style="margin-bottom: 20px; color: #333; text-align: center;">🏆 Group Rankings - Total Scores</h2>';
...
function renderOrtsverbandEvaluations(evaluations) {
    ...
    let html = '<h2 style="margin-bottom: 20px; color: #333; text-align: center;">🏆 Ortsverband Rankings - Total Scores</h2>';
    
    // Rankings table
    html += '<table class="group-table" style="max-width: 900px; margin: 0 auto;">';
    html += '<thead><tr>';
    html += '<th style="width: 100px; text-align: center;">Rank</th>';
    html += '<th>Ortsverband</th>';
    html += '<th style="width: 180px; text-align: center;">Stations Visited</th>';
    html += '<th style="width: 200px; text-align: center;">Total Score</th>';
    html += '</tr></thead><tbody>';
    ...
    evaluations.forEach((evaluation, index) => {
        ...
        html += '<td style="text-align: center; font-size: 16px;">' + evaluation.StationsVisited + '</td>';
        html += '<td style="text-align: center; font-weight: bold; font-size: 22px; color: #667eea;">' + evaluation.TotalScore + '</td>';
        html += '</tr>';
    });
    ...
    html += '<div class="stat-item">';
    html += '<strong>Highest Score</strong>';
    html += '<span>' + evaluations[0].TotalScore + ' (' + escapeHtml(evaluations[0].Ortsverband) + ')</span>';
    html += '</div>';
    ...
    html += '<div class="stat-item">';
    html += '<strong>Lowest Score</strong>';
    html += '<span>' + lastEval.TotalScore + ' (' + escapeHtml(lastEval.Ortsverband) + ')</span>';
    html += '</div>';
...
function renderStationTabs(stations, groups) {
    ...
    // Score dropdown (1200 to 100 in steps of 50)
    formHtml += '<label style="font-weight: 600; color: #333;">Score:</label>';
    formHtml += '<select id="global-score-select" style="padding: 10px; border-radius: 4px; border: 1px solid #ddd; font-size: 14px; min-width: 120px;">';
    formHtml += '<option value="">Select Score...</option>';
    for (let score = 1200; score >= 100; score -= 50) {
        formHtml += '<option value="' + score + '">' + score + '</option>';
    }
    formHtml += '</select>';
    
    // Station selector (shows which station to assign to)
    formHtml += '<label style="font-weight: 600; color: #333;">Station:</label>';
    formHtml += '<select id="global-station-select" style="padding: 10px; border-radius: 4px; border: 1px solid #ddd; font-size: 14px; min-width: 200px;">';
    stations.forEach((station, index) => {
        const selected = index === 0 ? ' selected' : '';
        formHtml += '<option value="' + station.StationID + '"' + selected + '>' + station.StationName + '</option>';
    });
    formHtml += '</select>';
...
function showStationDetails(stationIndex) {
    ...
    // Update station selector in the form
    const stationSelect = document.getElementById('global-station-select');
    if (stationSelect) {
